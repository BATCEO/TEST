...
Finally, for the weather overview, here is a pretty and useful visualisation idea I've come accross that shows daily values on a multi-year calendar view seqmented by the month. For creating this visualisation we can make use of the `augment` function of the `timetk` package which we introduced above.

When plotting the average temperature on a two-colour gradient we see clearly the summer months and are also able to identify small-scale fluctuations within a month or week.
...
foo %>%
  ...
  filter(!is.na(mweek2)) %>%
  mutate(wday = fct_relevel(wday, c("Sun", "Sat", "Fri", "Thu", "Wed", "Tue", "Mon"))) %>%
  ggplot(aes(mweek2, wday, fill = avg_temperature)) + 
  geom_tile(colour = "white") +
  scale_fill_gradient(low="dark blue", high="red") +
  facet_grid(year~month) + 
  labs(x="Week of Month",
       y="",
       title = "Average Temperature Heatmap", 
       fill="T_avg")
```

---

Many thanks for all the upvotes, comments, and support! You guys are awesome! Best of success with your predictions!
